window.loadGlobalStats = async function() {
    ...
    try {
        ...
        if (!data.guests || data.guests.length === 0) {
            guestsHtml += `<div class="text-center text-muted small py-3">Noch keine Gastspiele verzeichnet.</div>`;
        } else {
            data.guests.forEach((g, idx) => {
                ...
                guestsHtml += `
                    <div class="list-group-item bg-transparent text-white border-secondary border-opacity-10 d-flex justify-content-between align-items-center px-0 py-2.5">
                        ...
                        <span class="badge bg-primary bg-opacity-25 text-primary border border-primary border-opacity-25 rounded-pill px-3 py-1 fw-bold">${g.play_count} Partien</span>
                    </div>
                `;
            });
        }
        ...
        if ((!data.recent_popular || data.recent_popular.length === 0) && (!data.all_time_popular || data.all_time_popular.length === 0)) {
            popularHtml += `<div class="text-center text-muted small py-3">Noch keine Spiele aufgezeichnet.</div>`;
        } else {
            if (data.recent_popular && data.recent_popular.length > 0) {
                popularHtml += `<h6 class="text-white-50 x-small text-uppercase tracking-wider fw-bold mb-3 mt-1">🔥 Letzte 30 Tage</h6>`;
                    ...
                    popularHtml += `
                        <div class="d-flex align-items-center gap-3 mb-3">
                            ...
                            <div class="flex-grow-1 min-w-0">
                                ...
                                <span class="text-info x-small fw-semibold">${g.play_count}x gespielt</span>
                            </div>
                        </div>
                    `;
                });
            ...
            if (data.all_time_popular && data.all_time_popular.length > 0) {
                popularHtml += `<h6 class="text-white-50 x-small text-uppercase tracking-wider fw-bold mb-3 mt-4">🏆 All-Time Favoriten</h6>`;
                data.all_time_popular.forEach(g => {
                    ...
                    popularHtml += `
                        <div class="d-flex align-items-center gap-3 mb-3">
                            ...
                            <div class="flex-grow-1 min-w-0">
                                <span class="d-block text-white fw-bold text-truncate small">${g.name}</span>
                                <span class="text-warning x-small fw-semibold">${g.play_count}x gespielt</span>
                            </div>
                        </div>
                    `;
                });
        ...
        if (!data.never_played || data.never_played.length === 0) {
            unplayedHtml += `<div class="text-center text-muted small py-3">Alle Spiele in der Sammlung wurden mindestens einmal gespielt! 🎉</div>`;
        } else {
        ...
        if (!data.best_performances || data.best_performances.length === 0) {
            strengthsHtml += `<div class="text-center text-muted small py-2">Noch nicht genügend Daten.</div>`;
        } else {
                ...
                strengthsHtml += `
                    <div class="d-flex align-items-center justify-content-between p-2 rounded" style="background: rgba(46, 213, 115, 0.05); border: 1px solid rgba(46, 213, 115, 0.1);">
                        <div class="min-w-0 flex-grow-1">
                            ...
                            <span class="text-white-50 x-small text-truncate d-block">${p.game_name} (${p.games_played} Spiele)</span>
                        </div>
                        <span class="badge bg-success bg-opacity-25 text-success rounded px-2.5 py-1.5 fw-bold ms-2" style="font-size: 0.8rem;">${rate}% Wins</span>
                    </div>
                `;
            });
        }
        ...
        if (!data.worst_performances || data.worst_performances.length === 0) {
            weaknessesHtml += `<div class="text-center text-muted small py-2">Noch nicht genügend Daten.</div>`;
        } else {
            data.worst_performances.forEach(p => {
                ...
                weaknessesHtml += `
                    <div class="d-flex align-items-center justify-content-between p-2 rounded" style="background: rgba(255, 71, 87, 0.05); border: 1px solid rgba(255, 71, 87, 0.1);">
                        ...
                        <span class="badge bg-danger bg-opacity-25 text-danger rounded px-2.5 py-1.5 fw-bold ms-2" style="font-size: 0.8rem;">${rate}% Wins</span>
                    </div>
                `;
            });
        }
            ...
            <div class="row g-4">
                ...
                <div class="col-12 col-md-6 animate-fade-in" style="animation-delay: 0.1s;">
                    <div class="card border-0 h-100 p-4 shadow-sm" style="background: var(--surface-card); border-radius: 1.2rem; border: 1px solid var(--surface-border) !important;">
                        ...
                        <h6 class="text-success x-small text-uppercase tracking-wider fw-bold mb-3">🔥 Am besten in...</h6>
                        ${strengthsHtml}
                        <h6 class="text-danger x-small text-uppercase tracking-wider fw-bold mb-3 mt-2">❄️ Ausbaufähig in...</h6>
                        ${weaknessesHtml}
                    </div>
                </div>
                ...
                <div class="col-12 col-md-6 animate-fade-in" style="animation-delay: 0.3s;">
                    <div class="card border-0 h-100 p-4 shadow-sm" style="background: var(--surface-card); border-radius: 1.2rem; border: 1px solid var(--surface-border) !important;">
                        <h5 class="fw-bold text-white mb-3 d-flex align-items-center gap-2">
                            <span>👥 Gast-Aktivität</span>
                        </h5>
                        <p class="text-white-50 small mb-3">Welche Gäste haben am häufigsten mitgespielt?</p>
                        ${guestsHtml}
                    </div>
                </div>
                ...
                <div class="col-12 col-md-6 animate-fade-in" style="animation-delay: 0.4s;">
                    <div class="card border-0 h-100 p-4 shadow-sm" style="background: var(--surface-card); border-radius: 1.2rem; border: 1px solid var(--surface-border) !important;">
                        <h5 class="fw-bold text-white mb-3 d-flex align-items-center gap-2">
                            <span>📦 Ungespielte Schätze</span>
                        </h5>
                        <p class="text-white-50 small mb-3">Spiele aus deiner Sammlung mit 0 aufgezeichneten Partien:</p>
                        ${unplayedHtml}
                    </div>
                </div>
            </div>
        ...
        container.innerHTML = `
            <div class="alert alert-danger rounded-3 fw-bold small text-center" role="alert">
                Fehler beim Laden der globalen Statistiken.
            </div>
        `;
    }
};
