window.searchBGG = async function() {
    ...
    document.getElementById('searchResults').innerHTML = `<div class="text-center text-primary small p-3 spinner-border spinner-border-sm" role="status"></div><span class="small text-muted ms-2">${t('label_searching', 'Suche...')}</span>`;
    const res = await authFetch(`/search?name=${q}`); 
    if (!res) return;
    const data = await res.json();
    if(data.results.length === 0) { document.getElementById('searchResults').innerHTML = `<div class="text-center text-muted small p-3">${t('msg_nothing_found', 'Nichts gefunden.')}</div>`; return; }
    document.getElementById('searchResults').innerHTML = data.results.map(g => `
        <div class="list-group-item d-flex justify-content-between align-items-center animate-fade-in rounded-3 mb-2" onclick="previewGame(${g.id}, '${g.name.replace(/'/g, "\\'")}')" style="cursor:pointer; background: rgba(255,255,255,0.05); border: 1px solid var(--surface-border);">
            <span class="small fw-bold text-white">${g.name}</span><span class="badge bg-primary rounded-pill px-3 py-2 shadow-sm">${t('btn_preview', 'VORSCHAU')}</span>
        </div>`).join('');
};
...
window.previewGame = async function(bggId, tempName) {
    ...
    try {
        ...
        else { document.getElementById('previewCoverImage').innerHTML = `<div class="p-4 text-center text-muted bg-dark">${t('msg_no_cover', 'Kein Cover')}</div>`; }
        let badgesHtml = '';
        ...
        if (data.playing_time) badgesHtml += `<span class="badge bg-info bg-opacity-25 text-info border border-info border-opacity-50 rounded-pill px-2 py-1">⏱ ${data.playing_time} ${t('label_minutes_short', 'Min')}</span>`;
    ...
    } catch (e) { document.getElementById('previewCoverImage').innerHTML = `<div class="p-4 text-center text-danger bg-dark">${t('msg_error', 'Fehler')}</div>`; }
};
