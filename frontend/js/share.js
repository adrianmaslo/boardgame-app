...
window.openShareImageModal = function(session) {
    ...
    setTimeout(() => {
        generateShareCanvas(session, (dataUrl, blob) => {
            ...
            if (downloadBtn) {
                ...
                downloadBtn.download = `${session.game_name.replace(/\s+/g, '_')}_runde.png`;
            }
        });
    }, 400);
};
...
function generateShareCanvas(session, callback) {
    ...
    let displayName = session.game_name;
    ...
    const playDate = new Date(session.play_date).toLocaleDateString(dateLocale, { 
    ...
    scores.sort((a, b) => {
        ...
        if (session.win_condition === 1) {
            return a.score_value - b.score_value; // niedrigere Punkte besser
        }
        return b.score_value - a.score_value; // höhere Punkte besser
    });
    ...
    scores.forEach((sc, idx) => {
        ...
        ctx.fillText(sc.name, cardX + 70, cardY + cardHeight / 2);
        ...
        if (session.win_condition !== 2) {
            ...
            ctx.fillText(sc.score_value.toString(), cardX + cardWidth - 25, cardY + cardHeight / 2);
        } else {
            if (sc.is_winner) {
                ctx.fillStyle = '#eab308';
                ctx.font = 'bold 16px system-ui, -apple-system, sans-serif';
                ctx.fillText(t('label_winner_cap', '🏆 SIEGER'), cardX + cardWidth - 25, cardY + cardHeight / 2);
            }
        }
    });
    ...
    // Callback mit dataURL und blob aufrufen
